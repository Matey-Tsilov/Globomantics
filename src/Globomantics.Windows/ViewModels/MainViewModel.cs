    ...
    IViewModel
{
    ...
    public ObservableCollection<Todo> Complated { get; set; } = new();
        ...
        IRepository<TodoTask> todoRepository)
    {
        WeakReferenceMessenger.Default.Register<TodoSavedMessage>(this, (sender, message) =>
        {
            ...
            if (item.IsCompleted)
            {
                ...
                ReplaceOrAdd(Complated, item);
            }
            else
            {
                var existing = Complated.FirstOrDefault(i => i.Id == item.Id);

                if (existing is not null)
                {
                    Complated.Remove(existing);
                }
    ...
    private async Task ImportAsync()
    {
        ...
        {
            ShowError?.Invoke("No file specified");
        }
    ...
    public async Task InitializeAsync()
    {
        ...
        App.CurrentUser = await userRepository.FindByAsync("Matey");
